...
res = [];
p = [];
path = ('C:\Users\wangqi\Documents\Lab\Data\hang_fmri_data05302019\26');
...
% import path and fid if number of files is multiple
    if NFiles > 1
        fid = BrReadFid(path{CntFile});
        % Read the parameters
        pathstr = fileparts(path{CntFile});
% if not existing much files, import 1st element of path cell
    else
        if iscell(path)
            path = path{1};
        end
...
% acqfile is in pattern of pathstr+fileseq+'acqp'
    acqpfile = [pathstr,filesep,'acqp'];
    disp(['Reading parameter file ',acqpfile]);
% Pass each parameter from files to P
    p = BrReadParams(pnames,acqpfile);
% here p.ACQ_ReceiveSelect name of params
    if numel(p.ACQ_ReceiverSelect) >1
% count how many 'Yes' in 'ACQ_ReceiverSelect'
        NReceivers = sum(strcmp(p.ACQ_ReceiverSelect,'Yes'));
    else
        NReceivers = 1;
    end
% Count 'ACQ_size' and make last element '1'
...
% find out value which numbers in numbers > datasize
            qq = find(numbers > datasize);
            qq = qq(1);
            datasize = numbers(qq);
     end 
% use multiple dimension to re-define 'fid', e.g. a excel pattern list.
