...
%###########################Qi's Editing#################################
%% Checking input and format of parameters
if nargin < 1
...
% params should be in format of 'char' or 'cell'
...
% only read variable 'params' as 'char' format
...
line = 0;
% create a set full of 0, numel(params) denotes number of elements in
% params. 
paramfound = zeros(numel(params),1);
if numel(params)>1
    for cnt = 1: numel(params)
% passing params in array to output arr
        arr.(params{cnt})={};
    end
end
%% Read file line by line
while line ~= -1
% take out each lines with '\n'
  line = fgets(file);
  if line ~= -1
% delete pre-or-post blanck of string
    line = strtrim(line);
% Does the line contain a parameter name?
    if (strfind(line, '##$'))==1
% Take out the string before symbol '=' varied from 4th line to 'pname'
% Take the rest of string to 'pvalue'
% pname would be type/name of params, pvalue is value of corresponding
% params
        [pname, pvalue] = strtok(line(4:end), '=');
%Does this parameter appear in the params-array?
        nfoundparams = 0;
          for cnt=1:numel(params)
% compare if two strings are same, ignore Upper/Lower case
